...
MEMORY_SIZE = 2048
BATCH_SIZE = 128
C_STEPS = 100  # every C steps reset Q_ = Q
...
class DQN():
    ...
    def __init__(self, state_num, action_num, epsilon=0.9, gamma=0.9):
        ...
        self.QNet_ = Net(state_num, action_num)
        self.step = 0
    ...
    def learn(self):
        ...
        if self.step % C_STEPS == 0:
            self.QNet_.load_state_dict(self.QNet.state_dict())
        self.step += 1
        ...
        next_state_values[non_terminal_mask] = self.QNet_(non_terminal_state_next).detach().max(1)[0]
...
class CartPoleAgent():
    ...
    def __init__(self, epsilon=0.95, gamma=0.99, episode=1200):
    ...
    def saveImg(self, x, r, l):
        ...
        plt.xlabel('step')
        plt.plot(x, r)
        plt.savefig('CartPoleI.png')

    def dump(self, file='CartPoleNet.pkl'):
        torch.save(self.DQN.QNet, file)

    def load(self, file='CartPoleNet.pkl'):
...
class MountainCarAgent():
    def __init__(self, epsilon=0.90, gamma=0.99, episode=500):
    ...
    def saveImg(self, x, r, l):
        ...
        plt.xlabel('step')
        plt.plot(x, r)
        plt.savefig('MountainCarI.png')

    def dump(self, file='MountainCarNet.pkl'):
        torch.save(self.DQN.QNet, file)

    def load(self, file='MountainCarNet.pkl'):
        if os.path.exists(path=file):
            self.DQN.QNet = torch.load(file)
            return True
        else:
...
class AcrobotAgent():
    ...
    def saveImg(self, x, r, l):
        ...
        plt.xlabel('step')
        plt.plot(x, r)
        plt.savefig('AcrobotI.png')

    def dump(self, file='AcrobotNet.pkl'):
        torch.save(self.DQN.QNet, file)

    def load(self, file='AcrobotNet.pkl'):
        if os.path.exists(path=file):
            self.DQN.QNet = torch.load(file)
            return True
        else:
